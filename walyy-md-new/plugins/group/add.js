export default {
  ...
  run: async (conn, msg, {
    ...
    if (!isGroup) {
      return conn.sendMessage(chatId, { text: "Perintah ini hanya bisa digunakan dalam grup sukii" });
    }

    const { botAdmin, userAdmin } = await exGrup(conn, chatId, senderId);

    if (!userAdmin) {
      return conn.sendMessage(chatId, { text: "Kamu bukan Admin kontol bgst" }, { quoted: msg });
    }
    if (!botAdmin) {
